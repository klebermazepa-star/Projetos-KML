...
**       DATA....: JUNHO DE 1997
**
**       OBJETVO.: INTEGRA��O COM CONTAS A PAGAR
**
...
{utp/ut-glob.i}
{cdp/cd0666.i} /**** Defini��o da temp-table tt-erro ****/
{rep/re0404.i1} /* Define temp-tables, frames e varivaeis e preprocessadores comuns */
{cdp/cd4300.i3}

{app/apapi001b.i} /*Defini��o de tem-tables */
...
{utp/ut-liter.i Integra��o_com_Contas_a_Pagar *}
...
/**********************************************************************
**      EMS - 5.0 Integra��o "On-line" Datasul-EMS 2.0
**********************************************************************/
...
do  on endkey undo, return
    ...
    &endif
       ...
       /***********************************************************************
        * Consist�ncia para somente chamar o re9340 para os documentos que o  *
       ...
       IF CAN-FIND(FIRST dupli-apagar {cdp/cd8900.i dupli-apagar docum-est}) OR
        ...
        ELSE DO:
          ...
          and docum-est.esp-docto = 20     /* Devolu��o a fornecedor               */
          then do:  
          ...
          END. /*** Termino atualiza��o Nota Cr�dito ***/
        END.
...
/*       
    ...
    assign tt-doc-i-ap.cod-esp    = if param-global.modulo-ap = no 
                                    then "CT" /* Atualiza��o via Multiplanta             */ 
                                              /* Indica contabiliza��o na planta destino */
                                    else "" 
        ...
        if not connected("emsbas") or 
           ...
           not connected("emsuni") then do:          
            ...
            if  valid-handle (h-btb009za) AND
               ...
               run pi-conecta-bco IN h-btb009za (Input 1,                         /*contem a vers�o de integra��o da Api*/
                                                 Input i-conect,                  /*contem a op��o desejada (1-Conex�o, 2-Desconex�o)*/
            ...
            if connected("emsbas") or 
               ...
               connected("emsuni") then do:
                ...
                if  valid-handle (h-btb009za) AND
                   ...
                   run pi-conecta-bco IN h-btb009za (Input 2,                         /*contem a vers�o de integra��o da Api*/
                                                     Input i-conect,                  /*contem a op��o desejada (1-Conex�o, 2-Desconex�o)*/
                                                     Input param-global.empresa-prin, /*contem o c�digo da empresa*/
                                                     Input "all",                     /*contem o c�digo do banco externo*/ 
                                                     Output Table tt_erros_conexao).  /*retorna erros caso existam*/
